...
JSON_Serializer.prototype.serialize = function(data, serialization_schema, bundle){
	...
	if(Array.isArray(data)){
		...
		for(var i = 0; i < data.length; i++){
			var item = data[i];
			result.push(this.serialize(item, serialization_schema, bundle));
		}

	}else if(typeof(data) === 'object'){
		result = this.serialize_object(data, serialization_schema, bundle);
	}

	return result;
};

JSON_Serializer.prototype.serialize_object = function(data, serialization_schema, bundle){
	if(typeof(serialization_schema) !== 'object' || Array.isArray(serialization_schema)
		|| serialization_schema === undefined || serialization_schema === null){
		throw 'serialization_schema is not valid object.';
	}	
	...
	for (var key in serialization_schema) {
		if (data.hasOwnProperty(key) && serialization_schema.hasOwnProperty(key)) {
			...
			if(typeof(data[key]) === 'object' &&
				...
				result[key] = this.serialize(data[key], serialization_schema[key][this.keywords['SHOW']], bundle);
				continue;
			}
	    	...
	    	if(typeof(set) === 'function'){set = set(data, bundle);}
	    	if(typeof(show) === 'function'){show = show(data, bundle);}
	    	if(typeof(as) === 'function'){as = as(data, bundle);}
	...
	for(var field in serialization_schema){
		if (serialization_schema.hasOwnProperty(field)){
			...
			if(typeof(value) === 'function'){
				...
				var set = undefined;
				var as = undefined;
				var minimize = false; //don't minimize by default
